...
def generate_aitken_schema(x, y):
    ...
    for j in range(1, len(aitken_schema[0])):
        for i in range(j + 1, len(aitken_schema)):
            ...
            aitken_schema[i][j] = (aitken_schema[i][j - 1] - aitken_schema[i - 1][j - 1]) / (x[i] - x[0])
    print(aitken_schema)
    return np.insert(np.diagonal(aitken_schema, -1), 0, y[0])


def compute_L_polynom(value, x_array, y_array):
    Ln = y_array[0]
    right_term = 1
...
if __name__ == '__main__':
    ...
    print(f1(x_1_0))
    points = generate_points_from_interval(x_1_0, x_1_n, 10)
    points = [1, 2, 3, 4, 5]
